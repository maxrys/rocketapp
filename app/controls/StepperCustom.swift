...
struct StepperCustom<T>: View where T: Numeric & Comparable {
    ...
    public var body: some View {
        HStack(spacing: 10) {
            ...
            self.button(image: Image(systemName: "minus.circle"), disabled: self.value == self.range.lowerBound) {
            ...
            self.button(image: Image(systemName: "plus.circle"), disabled: self.value == self.range.upperBound) {
                let newValue = self.value + self.step
                self.value = newValue.fixBounds(
                    min: self.range.lowerBound,
                    max: self.range.upperBound
    ...
    @ViewBuilder private func button(image: Image, disabled: Bool, onClick: @escaping () -> Void) -> some View {
        ButtonRound(
            label     : { image },
