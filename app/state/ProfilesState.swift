...
@Observable final class ProfilesState {
    ...
    @ObservationIgnored var list: [ProfileID: ProfileTitle] {
        self.cache.reduce(into: [ProfileID: ProfileTitle]()) { (result, item) in
            if (item.value.ID == ThisApp.EMBEDDED_PROFILE_ID)
                 { result[item.value.ID] = item.value.title + " | id: \(item.value.ID) | " + NSLocalizedString("embedded", comment: "") }
            else { result[item.value.ID] = item.value.title + " | id: \(item.value.ID)" }
        }
    }
